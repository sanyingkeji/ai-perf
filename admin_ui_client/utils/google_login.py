...
- 下载得到的 client_secret JSON，命名为 `google_client_secret.json`，
  放到 admin_ui_client 目录下（与 main.py 同级）
...
def _open_in_new_window(auth_url: str):
    # macOS 上可以用 AppleScript 控制 Chrome/Safari 尺寸
...
def _get_client_secret_file() -> Path:
    ...
    if not path.exists():
        raise GoogleLoginError(
            ...
            f"请将 Google 控制台下载的 client_secret JSON 命名为 google_client_secret.json 放在 admin_ui_client 根目录下。"
        )
    return path
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>Ai Perf 管理端 - 让绩效管理公平、透明、自动化</p>
        </div>
    </div>
    <script>
        setTimeout(function() {
...
def login_and_get_id_token(callback_received_callback=None) -> str:
    ...
    try:
        ...
        # 让库自己选择可用端口（port=0），避免固定端口被占用的问题
        # 通过继承和替换来美化回调页面
        try:
            ...
            creds = flow.run_local_server(
                ...
                success_message="登录成功，可以回到 Ai Perf 管理端客户端。",
    ...
    # 第二步：调用后端 /admin/auth/google_login 换取 session_token
    base_url = (cfg.get("api_base") or "http://127.0.0.1:8880").rstrip("/")
    url = f"{base_url}/admin/auth/google_login"
    ...
    try:
        ...
        if resp.status_code != 200:
            raise GoogleLoginError(f"后端登录失败：HTTP {resp.status_code} - {resp.text}")
        raise GoogleLoginError(f"解析后端登录响应失败：{e}") from e

    # 检查响应状态（管理端登录接口返回 status="error" 时，HTTP 状态码仍然是 200）
    status = data.get("status", "")
    if status == "error" or resp.status_code != 200:
        message = data.get("message") or data.get("detail") or str(data)
        message_str = str(message)
        
        # 检查是否是权限相关的错误（管理端邮箱不在管理员列表中）
        if (resp.status_code == 403 or 
            "只有以下管理员邮箱" in message_str or 
            "ADMIN_EMAIL 未配置" in message_str or
            "权限" in message_str or 
            "permission" in message_str.lower() or 
            "unauthorized" in message_str.lower() or
            "forbidden" in message_str.lower()):
            raise GoogleLoginError(f"无权限：{message}\n\n只有配置的管理员邮箱才能登录管理端。")
        
        # 检查是否是超时错误（408），但可能是邮箱不在管理员列表中
        if resp.status_code == 408:
            # 408 超时错误，可能是数据库查询超时，但也可能是邮箱不在管理员列表中
            raise GoogleLoginError(f"登录超时：{message}\n\n如果您的邮箱不在管理员列表中，请联系管理员添加。")
        
        # 其他错误
        with _login_lock:
            _login_in_progress = False
        raise GoogleLoginError(f"后端登录失败：{message}")
